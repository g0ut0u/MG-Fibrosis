...
class MSNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # 全局平均池化
        cbam4 = self.gap(x4).flatten(1)       # [B, 64]
        cbam5 = self.gap(x5).flatten(1)       # [B,128]
        cbam1 = self.gap(u1).flatten(1)       # [B, 64]

        # 融合并分类
        feat = torch.cat([cbam4, cbam5, cbam1], dim=1)  # [B, 256]
...
if __name__ == "__main__":
    model = MSNet(in_channels=1, bilinear=True, dropout=0.3)
