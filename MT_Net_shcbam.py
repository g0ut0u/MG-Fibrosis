...
class GlobalPoolingClassifier(nn.Module):
    ...
    def forward(self, f1, f2, f3):
        # 对每个特征图进行 GAP，得到 [B, C, 1, 1] -> squeeze 为 [B, C]
...
class MTNet_sh(nn.Module):
    def __init__(self, in_channels=1, bilinear=True, dropout=0.3):
        ...
        self.stage4 = ResidualBlock(64, 64, stride=1, num_layers=4)             # H/8

        self.down45 = DownRes(64, 128, num_layers=1)
        self.stage5 = ResidualBlock(128, 128, stride=1, num_layers=5)           # H/16
        ...
        self.cls_head = GlobalPoolingClassifier(
    ...
    def forward(self, x, return_cls=True):
        ...
        cls_logits = self.cls_head(x4, x5, u1)
